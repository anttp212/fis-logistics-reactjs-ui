...
const UploadMinio: React.FC<Props> = ({ value = [], onChange, initialFileList, acceptOnlyDocuments = false }) => {
  const fileList = useMemo(
    () => toAntdFileList(value, initialFileList),
  ...
  const validateFile = (file: File): boolean => {
    ...
    if (!DOCUMENT_EXTENSIONS.includes(ext as any)) {
      message.error(
        `Chỉ chấp nhận file: ${DOCUMENT_EXTENSIONS.join(', ')}. File "${file.name}" không hợp lệ.`
      );
      return false;
    }
    return true;
  ...
  const uploadFile = async (file: File) => {
    try {
      ...
      const res = await axios.get(
        `${API_DEFAULTS.baseUrl}${API_ENDPOINTS.files.presignedUploadUrl}`,
        {
  ...
  return (
    <Dragger {...props}>
      ...
      <p className="ant-upload-text">
        Nhấn hoặc kéo thả file để tải lên
      </p>
